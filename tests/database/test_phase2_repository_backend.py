...
def test_supabase_manifest_retention_settings_upsert_requires_access_token() -> None:
    import app.db.phase2_store as phase2_store
...
def test_supabase_manifest_retention_settings_get_uses_user_scoped_rest_when_access_token_present(monkeypatch) -> None:
    import app.db.phase2_store as phase2_store
    ...
    record = repo.get("museum-org", access_token="jwt-token")

    assert captured["table_name"] == "org_data_retention_settings"
    assert captured["params"] == {
        "select": "org_id,plan_tier,manifest_retention_days,manifest_redaction_enabled,updated_by,created_at,updated_at",
        "org_id": "eq.museum-org",
        "plan_tier": "eq.museum",
        "limit": "1",
    }
    assert captured["headers"] == {"Authorization": "Bearer jwt-token"}
    assert record is not None
    assert record["manifest_retention_days"] == 90
    assert record["manifest_redaction_enabled"] is True
