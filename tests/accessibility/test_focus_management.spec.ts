...
function buildCompletedJob() {
  return {
    job_id: "job-focus-1",
    media_uri: "gs://chronos-test-bucket/uploads/focus-user/upload-1/archive.mov",
    ...
    progress_topic: "job_progress:job-focus-1",
    result_uri: "gs://chronos-test-bucket/results/job-focus-1/result.mp4",
...
describe("Packet 4D focus management", () => {
  beforeEach(() => {
    ...
    executeUploadFlow.mockImplementation(async ({ handlers }) => {
      ...
      handlers.setUploadSession({
        ...
        object_path: "uploads/focus-user/upload-1/archive.mov",
        media_uri: "gs://chronos-test-bucket/uploads/focus-user/upload-1/archive.mov",
    ...
    fetchUncertaintyCallouts.mockResolvedValue({ job_id: "job-focus-1", status: "completed", callouts: [] });
    fetchCurrentUserProfile.mockResolvedValue({ user_id: "focus-user", plan_tier: "museum" });
  });

  it("moves focus to the terminal delivery heading when processing completes", async () => {
  ...
  it("moves focus to the delivery alert when an export action fails", async () => {
    const user = userEvent.setup();
    fetchJobExport.mockRejectedValue(Object.assign(new Error("The delivery package has expired and been deleted."), { status: 410 }));
    ...
    expect(alert).toHaveTextContent("The delivery package has expired and been deleted.");
  });
});
