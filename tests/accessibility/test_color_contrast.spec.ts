...
describe("Packet 4D color contrast states", () => {
  beforeEach(() => {
    ...
    saveUploadConfiguration.mockResolvedValue({
      ...
      relative_processing_time_band: "<4 min/min",
      job_payload_preview: {
        media_uri: "gs://chronos-test-bucket/uploads/contrast-user/upload-1/archive.mov",
    ...
    executeUploadFlow.mockImplementation(async ({ handlers }) => {
      ...
      handlers.setUploadSession({
        ...
        object_path: "uploads/contrast-user/upload-1/archive.mov",
        media_uri: "gs://chronos-test-bucket/uploads/contrast-user/upload-1/archive.mov",
    ...
    const completedJob = {
      ...
      estimated_duration_seconds: 180,
      status: "completed",
      progress_topic: "job_progress:job-contrast-1",
      result_uri: "gs://chronos-test-bucket/results/job-contrast-1/result.mp4",
    ...
    fetchUncertaintyCallouts.mockResolvedValue({ job_id: "job-contrast-1", status: "completed", callouts: [] });
    fetchCurrentUserProfile.mockResolvedValue({ user_id: "contrast-user", plan_tier: "pro" });
    fetchJobExport.mockRejectedValue(Object.assign(new Error("The delivery package has expired and been deleted."), { status: 410 }));
  });

  it("uses distinct high-visibility styles for export buttons and alerts", async () => {
    ...
    expect(av1Button).toHaveStyle({ color: "rgb(255, 255, 255)" });

    await user.click(av1Button);

    const alert = await screen.findByRole("alert");
    expect(alert).toHaveStyle({ background: "#fff0f0" });
    expect(alert).toHaveStyle({ color: "#8a1f1f" });
  });
});
