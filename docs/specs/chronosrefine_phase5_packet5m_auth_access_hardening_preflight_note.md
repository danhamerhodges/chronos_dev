...
2. Auth policy readbacks now declare SEC-001 session, cookie, password, lockout, Museum API-key, MFA, token-revocation, and auth-audit-event contracts.
...
- It does not implement hosted MFA enrollment, TOTP/SMS verification, backup-code recovery, or Supabase MFA runtime enforcement.
...
3. Hosted or integration evidence for MFA enrollment/enforcement, including Museum Admin enforcement.
4. Session-management evidence for secure cookie flags, token revocation, concurrent-session behavior, and session-fixation resistance.
