...
**Phase 3 Requirement Set:** **ENG-003**, **ENG-005**, **ENG-006**, **ENG-007**, **ENG-008**, **ENG-009**, **ENG-010**, **ENG-011**, **ENG-012**, **NFR-002**, **SEC-007**, **OPS-003** (12 requirements; `SEC-007` deferred to `GA+3 months`)
**Coverage Matrix:** See `docs/specs/ChronosRefine Requirements Coverage Matrix.md`

**Implementation Status Note:** Packets 3A, 3A.1, 3A.2, 3B, and 3C are merged to `main` via PR #2 (`a5b0f6c`). The docs-only follow-up on `main` is `5ac15cd`. Current staging/runtime evidence remains context-only and is tracked in `docs/specs/chronosrefine_phase3_closeout_note.md`. Phase 3 therefore stands at `11/12` complete on `main`, with **SEC-007** tracked as a deferred Museum-tier milestone for `GA+3 months`.
...
- [x] `SEC-007` is tracked as a deferred Museum-tier milestone and is not used as an unconditional Phase 3 completion blocker before `GA+3 months` → **Req:** **SEC-007**
...
**Current Status Note:** Packets 4A (`FR-001`), 4B (`FR-003` + `DS-001`), 4C (`FR-004` + the Packet 4C portion of `DS-006`), 4D (`FR-005` + `ENG-015`), 4E (`ENG-013` + the launch-time slice of `NFR-003`), 4F (`ENG-014`), 4G (`DS-002` + `DS-003` + `DS-004` + `DS-005`), and 4H (`NFR-003`) are merged on `main`, with Packet 4H landing in merge commit `61d51ff3ff289fc881674bd05afe37a8ac94cba0` on 2026-03-20. Phase 4 therefore stands at `14/14` complete on `main`. Packet 4H adds admin-only cost monitoring via `GET /v1/ops/costs`, tier-aware gross-margin reporting from current pricing plus per-job cost records, event-driven cost anomaly routing into the existing ops incident path, deterministic quarterly optimization recommendations, and cost-focused monitoring dashboard/alert updates without adding customer-facing billing UX. Packet 4E added `POST /v1/jobs/estimate`, launch-time cost review, single-job overage approval inside the launch modal, and per-job estimate/reconciliation summaries without marking `NFR-003` globally complete; Packet 4H closes the remaining Phase 4 operational slice on `main`. Follow-on packets must still follow the canonical Phase 4 requirement IDs above and the higher-priority requirement specs, not older PRD-style Phase 4 prose. `FR-006` remains a Phase 5 functional requirement; Phase 4 includes only `ENG-014` as the technical preview-generation substrate, and Phase 5 is next once its entry criteria are recorded.
...
-   Cost estimation and launch-time cost-optimization signals exposed at launch decision points without hardcoded pricing → **Req:** **ENG-013**, **NFR-003**, **Test:** `tests/api/test_cost_estimation.py`, `tests/api/test_cost_breakdown.py`, `tests/integration/test_cost_reconciliation.py`, `tests/ops/test_cost_optimization.py`, `tests/ui/test_cost_estimate_modal.spec.ts`, `tests/accessibility/test_cost_estimate_modal_a11y.spec.ts`, **See:** `docs/specs/chronosrefine_engineering_requirements.md#eng-013-cost-estimation`, `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-003-cost-optimization`
-   Preview artifact generation backend for downstream Phase 5 `FR-006` review flows, exposed as draft preview sessions keyed by `preview_id` rather than processing `job_id` → **Req:** **ENG-014**, **Test:** `tests/processing/test_preview_generation.py`, `tests/processing/test_scene_detection.py`, `tests/load/test_preview_performance.py`, **See:** `docs/specs/chronosrefine_engineering_requirements.md#eng-014-preview-generation`
...
- [x] Preview artifact generation backend meets `ENG-014` latency and storage expectations for downstream Phase 5 `FR-006` UI work on `main`, using owner-scoped draft preview sessions keyed by `preview_id` → **Req:** **ENG-014**, **Test:** `tests/processing/test_preview_generation.py`, `tests/processing/test_scene_detection.py`, `tests/load/test_preview_performance.py`, `tests/api/test_preview_sessions.py`, `tests/integration/test_preview_pipeline.py`, **See:** `docs/specs/chronosrefine_engineering_requirements.md#eng-014-preview-generation`
- [x] Accessibility audit passes WCAG 2.1 AA for `DS-002` through `DS-006` on the candidate branch for the implemented Phase 4 journey (upload, detection, configuration, launch review, processing/progress, export/delivery), with preview-review accessibility explicitly deferred to Phase 5 `FR-006`; rendered Packet 4G suites pass and the manual browser plus screen-reader closeout matrix is recorded → **Req:** **DS-002**, **DS-003**, **DS-004**, **DS-005**, **DS-006**, **Test:** `tests/accessibility/test_keyboard_navigation.spec.ts`, `tests/accessibility/test_keyboard_shortcuts.spec.ts`, `tests/accessibility/test_screen_reader_support.spec.ts`, `tests/accessibility/test_aria_labels.spec.ts`, `tests/accessibility/test_aria_live_regions.spec.ts`, `tests/accessibility/test_color_contrast.spec.ts`, `tests/accessibility/test_button_contrast.spec.ts`, `tests/accessibility/test_focus_contrast.spec.ts`, `tests/accessibility/test_focus_management.spec.ts`, `tests/accessibility/test_modal_focus.spec.ts`, `tests/accessibility/test_form_focus.spec.ts`, `tests/accessibility/test_error_messages.spec.ts`, **See:** `docs/specs/chronosrefine_design_requirements.md#ds-002-keyboard-navigation`, `docs/specs/chronosrefine_design_requirements.md#ds-003-screen-reader-support`, `docs/specs/chronosrefine_design_requirements.md#ds-004-color-contrast`, `docs/specs/chronosrefine_design_requirements.md#ds-005-focus-indicators`, `docs/specs/chronosrefine_design_requirements.md#ds-006-error-messages-accessibility`
- [x] Cost-estimate accuracy and Phase 4 cost-optimization guardrails align with `ENG-013` + `NFR-003` on `main` with no hardcoded pricing or entitlements. Packet 4E closes `ENG-013` on `main`; Packet 4H closes the remaining `NFR-003` ops slice on `main` by adding admin cost monitoring, anomaly routing, quarterly recommendations, and gross-margin reporting. → **Req:** **ENG-013**, **NFR-003**, **Test:** `tests/api/test_cost_estimation.py`, `tests/api/test_cost_breakdown.py`, `tests/integration/test_cost_reconciliation.py`, `tests/ops/test_cost_tracking.py`, `tests/ops/test_cost_optimization.py`, `tests/ops/test_gpu_utilization.py`, `tests/ops/test_monitoring.py`, **See:** `docs/specs/chronosrefine_engineering_requirements.md#eng-013-cost-estimation`, `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-003-cost-optimization`
- [x] Cross-browser coverage is complete for upload, configuration, launch, and export flows, with the Packet 4G candidate-branch manual verification matrix recorded in `docs/specs/chronosrefine_phase4_closeout_note.md` as historical evidence carried forward on `main` → **Req:** **DS-001**, **DS-002**, **DS-003**, **DS-004**, **DS-005**, **DS-006**, **See:** `docs/specs/chronosrefine_design_requirements.md#ds-001-fidelity-configuration-ux`, `docs/specs/chronosrefine_phase4_closeout_note.md`
...
**Phase 5 Requirement Set:** **FR-006**, **SEC-001**, **SEC-002**, **SEC-003**, **SEC-004**, **SEC-005**, **SEC-006**, **NFR-004**, **NFR-005**, **NFR-006**, **NFR-009** (11 requirements)
**Coverage Matrix:** See `docs/specs/ChronosRefine Requirements Coverage Matrix.md`
...
**Current Gate Status Note:** Phase 5 entry criteria are now satisfied in merged canon via the recorded pricing and legal/DPA gate notes. Packet 5A hosted-closeout evidence remains recorded in `docs/specs/chronosrefine_phase5_packet5a_closeout_note.md`, including `chronos_dev` migration proof through `0024`, Secret Manager/IAM unblock evidence for `STRIPE_SECRET_KEY`, hosted smoke, publish-failure retry proof, `/v1/jobs` non-regression, and hosted preview latency `p95=1.8466s`. Packet 5B now adds the global `FR-006` hosted-closeout evidence in `docs/specs/chronosrefine_phase5_packet5b_closeout_note.md`: generic public `/v1/jobs` now requires approved-preview provenance, malformed provenance still returns `422`, refreshed approved payloads succeed through the shared launch path, repeated generic launch returns the same `job_id`, stale anti-replay and cross-user denial remain enforced, publish-failure retry reuses the same bound `job_id`, `/v1/jobs/estimate` billing behavior remains unchanged, runtime verifier hygiene remains clean, and hosted preview latency stayed within the canonical `<6s p95` guardrail (`p95=0.9314s`). Global `FR-006` is therefore complete, canonical `NFR-008` ownership still remains Phase 6 work, and the public API migration path for legacy bare `/v1/jobs` clients is: refresh the saved configuration, approve the current preview, then relaunch with the new payload carrying `launch_context.source = approved_preview`. Packet 5C records the first hosted-complete `NFR-006` slice in `docs/specs/chronosrefine_phase5_packet5c_closeout_note.md`: runtime commercial pricebook resolution is active on current merged pricing-model surfaces, effective pricing is exposed through usage and estimate responses, a config-only pricebook change was applied and validated in `chronos_dev` without a code deploy, Packet 5A/5B non-regression remained green, runtime signal evidence was recorded, and hosted preview latency remained within the canonical guardrail (`p95=1.0161s`). Packet 5D then closes global `NFR-006` in `docs/specs/chronosrefine_phase5_packet5d_closeout_note.md`: hosted DB-backed audited pricebook revisions are active, exactly one revision remains active, later activation changes effective pricing without a code deploy, org-scoped billing summary and portal-session APIs are live, `/v1/webhooks/stripe` is replay-safe, Museum recurring pricing takes precedence over quote context, launch-time pricing snapshots remain immutable, margin reporting uses snapshots after later pricing changes, commercial config is supplied through Secret Manager refs, and final hosted logs do not carry raw Stripe secrets, webhook payloads, or raw commercial config values. Packet 5K-E5 reconciliation keeps global `SEC-005` open in `docs/specs/chronosrefine_phase5_packet5ke5_closeout_reconciliation_note.md` because the retention settings API/UI gate and durable approval evidence remain unresolved. Packet 5K-F records the missing retention settings API/UI gate in `docs/specs/chronosrefine_phase5_packet5kf_retention_settings_gate_preflight_note.md` and keeps global `SEC-005` open pending durable compliance and two-engineer approval evidence. Packet 5L records the Phase 5/Phase 6 governance boundary repair in `docs/specs/chronosrefine_phase5_packet5l_governance_boundary_repair_note.md` without closing any requirement. Phase 5 remains `2/11` full requirements complete.
...
-   Authentication and authorization controls hardened for tiered access → **Req:** **SEC-001**, **SEC-004**, **Test:** `tests/security/test_authorization.py`, `tests/security/test_rate_limiting.py`, `tests/security/test_multi_tenancy.py`, **See:** `docs/specs/chronosrefine_security_operations_requirements.md#sec-001-authentication--authorization`, `docs/specs/chronosrefine_security_operations_requirements.md#sec-004-access-control`
-   Platform-managed encryption and TLS controls validated, with deferred customer-managed key delivery kept in `SEC-007` (`GA+3 months`) → **Req:** **SEC-002**, **Test:** `tests/security/test_encryption_at_rest.py`, `tests/security/test_encryption_in_transit.py`, `tests/security/test_tls_configuration.py`, **See:** `docs/specs/chronosrefine_security_operations_requirements.md#sec-002-data-encryption`
...
-   GDPR support implemented for Article 17 workflow, DPA/compliance documentation, and deletion-proof substrate without closing Phase 6 `SEC-010` → **Req:** **SEC-006**, **Test:** `tests/compliance/test_gdpr_compliance.py`, `tests/compliance/test_gdpr_log_deletion.py`, `tests/api/test_deletion_proof.py`, **See:** `docs/specs/chronosrefine_security_operations_requirements.md#sec-006-gdpr-compliance`
-   Reliability/availability controls and Museum-tier DR posture validated, while launch runbooks and rollback monitoring remain Phase 6 `OPS-004` → **Req:** **NFR-004**, **NFR-005**, **Test:** `tests/ops/test_availability_slo.py`, `tests/ops/test_database_performance.py`, `tests/ops/test_job_retry.py`, `tests/load/test_scalability.py`, `tests/ops/test_disaster_recovery.py`, **See:** `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-004-reliability--availability`, `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-005-museum-sla--disaster-recovery`
...
-   UTF-8 metadata support and i18n framework foundations implemented without making Phase 6 `SEC-012` residency validation a prerequisite → **Req:** **NFR-009**, **Test:** `tests/i18n/test_utf8_encoding.py`, `tests/i18n/test_ui_translation.py`, `tests/i18n/test_localization.py`, **See:** `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-009-internationalization-i18n`

**Exit Criteria:**
- [x] Preview generation remains <6s p95 under expected load → **Req:** **FR-006**, **Test:** `tests/load/test_preview_performance.py`, **See:** `docs/specs/chronosrefine_functional_requirements.md#fr-006-preview-generation`, `docs/specs/chronosrefine_phase5_packet5b_closeout_note.md`
- [ ] Authentication/authorization and access control pass security validation suite → **Req:** **SEC-001**, **SEC-004**, **Test:** `tests/security/test_authorization.py`, `tests/security/test_rate_limiting.py`, `tests/security/test_multi_tenancy.py`, **See:** `docs/specs/chronosrefine_security_operations_requirements.md#sec-001-authentication--authorization`, `docs/specs/chronosrefine_security_operations_requirements.md#sec-004-access-control`
- [ ] Platform-managed encryption controls pass at-rest and in-transit verification, with full CMEK delivery left to deferred `SEC-007` → **Req:** **SEC-002**, **Test:** `tests/security/test_encryption_at_rest.py`, `tests/security/test_encryption_in_transit.py`, `tests/security/test_tls_configuration.py`, **See:** `docs/specs/chronosrefine_security_operations_requirements.md#sec-002-data-encryption`
...
- [ ] GDPR workflows validated for Article 17 support, DPA/compliance documentation, PII log deletion, and deletion-proof substrate without closing Phase 6 `SEC-010` → **Req:** **SEC-006**, **Test:** `tests/compliance/test_gdpr_compliance.py`, `tests/compliance/test_gdpr_log_deletion.py`, `tests/api/test_deletion_proof.py`, **See:** `docs/specs/chronosrefine_security_operations_requirements.md#sec-006-gdpr-compliance`
- [ ] Reliability/SLA/DR requirements validated for Museum tier, with finalized launch monitoring runbooks reserved for Phase 6 `OPS-004` → **Req:** **NFR-004**, **NFR-005**, **Test:** `tests/ops/test_availability_slo.py`, `tests/ops/test_database_performance.py`, `tests/ops/test_job_retry.py`, `tests/load/test_scalability.py`, `tests/ops/test_disaster_recovery.py`, **See:** `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-004-reliability--availability`, `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-005-museum-sla--disaster-recovery`
- [x] Pricing model and feature gating validate against Stripe-configured pricing only → **Req:** **NFR-006**, **NFR-012**, **Test:** `tests/billing/test_feature_gating.py`, `tests/billing/test_subscription_lifecycle.py`, **See:** `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-006-pricing-model`, `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-012-payment-provider-selection`, `docs/specs/chronosrefine_phase5_packet5d_closeout_note.md`
- [ ] UTF-8 handling and localization framework pass baseline coverage → **Req:** **NFR-009**, **Test:** `tests/i18n/test_utf8_encoding.py`, `tests/i18n/test_ui_translation.py`, `tests/i18n/test_localization.py`, **See:** `docs/specs/chronosrefine_nonfunctional_requirements.md#nfr-009-internationalization-i18n`
...
**Packet 5B hosted closeout note:** `docs/specs/chronosrefine_phase5_packet5b_closeout_note.md`

**Packet 5B outcome note:** Packet 5B closes global `FR-006` by extending approved-preview enforcement to public `/v1/jobs`, preserving Packet 5A preview-route behavior, and documenting the intentional migration path for legacy bare `/v1/jobs` clients. Phase 5 now records `1/11` full requirements complete while canonical `NFR-008` ownership remains in Phase 6.

**Packet 5C kickoff note:** `docs/specs/chronosrefine_phase5_packet5c_kickoff.md`

**Packet 5C execution packet:** `docs/specs/chronosrefine_phase5_packet5c_execution_packet.md`

**Packet 5C hosted closeout note:** `docs/specs/chronosrefine_phase5_packet5c_closeout_note.md`

**Packet 5C outcome note:** Packet 5C is hosted-complete as the first `NFR-006` slice for current merged pricing-model surfaces only. It adds runtime pricebook-backed pricing and entitlement resolution, additive effective-pricing transparency, and config-only hosted change proof in `chronos_dev` without a code deploy. Deferred `NFR-006` acceptance criteria remain open, so global `NFR-006` stays partial and the Phase 5 full-requirement count remains unchanged at `1/11`.

**Packet 5D hosted closeout note:** `docs/specs/chronosrefine_phase5_packet5d_closeout_note.md`

**Packet 5D outcome note:** Packet 5D closes global `NFR-006` by completing the remaining Stripe billing lifecycle, audited DB-backed pricebook revision, Museum override, immutable launch-pricing snapshot, margin-reporting, and hosted log-hygiene evidence. Packet 5D advanced Phase 5 to `2/11` full requirements complete.

**Packet 5K-E5 reconciliation note:** Packet 5K-E5 keeps global `SEC-005` open after PR review because `DoD-SEC-005-10` retention settings API/UI evidence and durable `DoD-SEC-005-11` approval evidence remain unresolved. Phase 5 remains `2/11` full requirements complete.

**Packet 5K-F retention settings gate preflight note:** Packet 5K-F implements the missing `DoD-SEC-005-10` API/UI configuration gate for `SEC-005` and records durable approval-evidence preflight requirements. It does not close `SEC-005`, does not record compliance or two-engineer approval, and does not advance Phase 5 beyond `2/11`.
...
**Phase 6 Requirement Set:** **FR-007**, **NFR-001**, **SEC-015**, **NFR-008**, **NFR-010**, **SEC-009**, **SEC-010**, **SEC-011**, **SEC-012**, **OPS-004** (10 requirements)
**Coverage Matrix:** See `docs/specs/ChronosRefine Requirements Coverage Matrix.md`
...
-   Deletion proof generation and verification finalized for launch readiness → **Req:** **SEC-010**, **Test:** `tests/api/test_deletion_proof.py`, **See:** `docs/specs/chronosrefine_security_operations_requirements.md#sec-010-deletion-proofs`
