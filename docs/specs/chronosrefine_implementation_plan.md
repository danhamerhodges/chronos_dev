...
- [x] Preview artifact generation backend meets `ENG-014` latency and storage expectations for downstream Phase 5 `FR-006` UI work on candidate branch `codex/packet4f-preview-substrate`, using owner-scoped draft preview sessions keyed by `preview_id` → **Req:** **ENG-014**, **Test:** `tests/processing/test_preview_generation.py`, `tests/processing/test_scene_detection.py`, `tests/load/test_preview_performance.py`, `tests/api/test_preview_sessions.py`, `tests/integration/test_preview_pipeline.py`, **See:** `docs/specs/chronosrefine_engineering_requirements.md#eng-014-preview-generation`
