...
**Current Status Note:** Packets 4A (`FR-001`), 4B (`FR-003` + `DS-001`), 4C (`FR-004` + the Packet 4C portion of `DS-006`), 4D (`FR-005` + `ENG-015`), 4E (`ENG-013` + the launch-time slice of `NFR-003`), and 4F (`ENG-014`) are merged on `main`, with Packet 4F landing in merge commit `aea4809b7cbb111e4273ece0a4fb566e96c63e7b`. Phase 4 therefore stands at `9/14` complete on `main`. Packet 4E adds `POST /v1/jobs/estimate`, launch-time cost review, single-job overage approval inside the launch modal, and per-job estimate/reconciliation summaries without marking `NFR-003` globally complete. Packet 4F adds draft preview-session creation/reread via `preview_id`, deterministic keyframe selection with cache reuse and uniform fallback, and keeps all Phase 5 `FR-006` review UX out of scope. The Packet 4D merge also delivers the export-flow slices of `DS-002`, `DS-003`, `DS-004`, and `DS-005` without marking those design requirements globally complete. Follow-on packets must still follow the canonical Phase 4 requirement IDs above and the higher-priority requirement specs, not older PRD-style Phase 4 prose. `FR-006` remains a Phase 5 functional requirement; Phase 4 includes only `ENG-014` as the technical preview-generation substrate, and Packet 4G is next.
