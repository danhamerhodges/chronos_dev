...
**Current Status Note:** Packets 4A (`FR-001`), 4B (`FR-003` + `DS-001`), 4C (`FR-004` + the Packet 4C portion of `DS-006`), 4D (`FR-005` + `ENG-015`), 4E (`ENG-013` + the launch-time slice of `NFR-003`), and 4F (`ENG-014`) are merged on `main`, with Packet 4F landing in merge commit `aea4809b7cbb111e4273ece0a4fb566e96c63e7b`. Phase 4 therefore stands at `9/14` complete on `main` and `13/14` complete on the candidate branch. Packet 4G (`DS-002` + `DS-003` + `DS-004` + `DS-005`) is complete on the candidate branch `codex/packet4g-accessibility-closeout`: the implemented upload, detection, configuration, launch review, processing/progress, and export/delivery accessibility work is on-branch, the automated Packet 4G accessibility gates pass locally, and the manual browser plus screen-reader closeout matrix is recorded in `docs/specs/chronosrefine_phase4_closeout_note.md`. Packet 4E adds `POST /v1/jobs/estimate`, launch-time cost review, single-job overage approval inside the launch modal, and per-job estimate/reconciliation summaries without marking `NFR-003` globally complete. Packet 4F adds draft preview-session creation/reread via `preview_id`, deterministic keyframe selection with cache reuse and uniform fallback, and keeps all Phase 5 `FR-006` review UX out of scope. Packet 4G keeps preview-review accessibility with Phase 5 `FR-006` while closing the remaining Phase 4 accessibility implementation on the current branch. Follow-on packets must still follow the canonical Phase 4 requirement IDs above and the higher-priority requirement specs, not older PRD-style Phase 4 prose. `FR-006` remains a Phase 5 functional requirement; Phase 4 includes only `ENG-014` as the technical preview-generation substrate, and Packet 4H is next after Packet 4G.
