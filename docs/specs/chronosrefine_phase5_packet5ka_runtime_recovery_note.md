...
- Post-merge recovery commit: `4f6c9c853ccc195015d4244f7cc24aa9431726ae`
...
The first recovered hosted revision was `chronos-phase1-app-00089-bnh`, serving `100%` of traffic with build SHA `0ac7328585819191c00c61427a722764ec48bb3b`. After PR merge, the staging workflow deployed `chronos-phase1-app-00090-7c4` but did not stamp build metadata because GitHub credentials made the checkout dirty before `emit_build_metadata.sh` ran. Packet 5K-A follow-up corrected the workflow to stamp `BUILD_SHA` from `GITHUB_SHA` and then performed a config-only metadata recovery to final serving revision `chronos-phase1-app-00091-mf7`.
...
- `.github/workflows/deploy-staging.yml` now calls `emit_build_metadata.sh` with `BUILD_SHA_OVERRIDE="${GITHUB_SHA}"` so credentials generated by GitHub authentication cannot make the checkout appear dirty before build metadata is stamped.
...
gcloud run services update chronos-phase1-app --project chronos-dev-489301 --region us-central1 --update-env-vars BUILD_SHA=4f6c9c853ccc195015d4244f7cc24aa9431726ae,BUILD_TIME=2026-05-24T01:50:45Z --quiet
...
- `latestCreatedRevisionName`: `chronos-phase1-app-00091-mf7`
- `latestReadyRevisionName`: `chronos-phase1-app-00091-mf7`
- service `Ready`: `True`
- service `ConfigurationsReady`: `True`
- service `RoutesReady`: `True`
- traffic: `100%` to `chronos-phase1-app-00091-mf7`
- public version after final metadata recovery:

```json
{"version":"0.2.0","build_sha":"4f6c9c853ccc195015d4244f7cc24aa9431726ae","build_time":"2026-05-24T01:50:45Z"}
```
