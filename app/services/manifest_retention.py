...
class ManifestRetentionService:
    ...
    def get_settings(self, *, org_id: str, plan_tier: str, access_token: str | None = None) -> dict[str, Any]:
        if plan_tier.lower() != "museum":
            raise ValueError("SEC-005 manifest retention settings are Museum-tier only.")
        record = self._settings.get(org_id, access_token=access_token)
        if record is None:
            return {
                "org_id": org_id,
                "plan_tier": "museum",
                ...
                "retention_class": RETENTION_CLASS_INDEFINITE,
                "updated_by": None,
                "updated_at": "",
            }
        retention_days = record["manifest_retention_days"]
        return {
            "org_id": record["org_id"],
            "plan_tier": record["plan_tier"],
